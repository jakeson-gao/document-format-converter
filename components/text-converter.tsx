...
export function TextConverter() {
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      if (extension === "txt") {
        conversionType = `txt-to-${outputFormat}` as ConversionType
      } else if (extension === "rtf") {
        conversionType = `rtf-to-${outputFormat}` as ConversionType
      } else if (extension === "html") {
        conversionType = `html-to-${outputFormat}` as ConversionType
      } else {
        conversionType = `${extension}-to-txt` as ConversionType
      }
  ...
  const getFormatOptions = () => {
    ...
    if (extension === "txt") {
      ...
      return [{ value: "txt", label: "Text (.txt)" }]
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold mb-2">Text Format Conversion</h2>
        <p className="text-slate-600 dark:text-slate-400">Convert between text formats like TXT, RTF, and HTML.</p>
      </div>

      <div className="space-y-4">
        <div>
          <Label htmlFor="text-file-upload" className="block mb-2">
            Upload File
          </Label>
          <div className="flex items-center gap-4">
            ...
            <input id="text-file-upload" type="file" className="hidden" onChange={handleFileChange} />
          </div>
        </div>
            ...
            >
              {getFormatOptions().map((option) => (
                <div key={option.value} className="flex items-center space-x-2">
                  <RadioGroupItem value={option.value} id={`text-${option.value}`} />
                  <Label htmlFor={`text-${option.value}`}>{option.label}</Label>
                </div>
              ))}
            </RadioGroup>
          </div>
